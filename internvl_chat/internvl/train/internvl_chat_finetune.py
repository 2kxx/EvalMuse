...
class LazySupervisedDataset(Dataset):
    ...
    def multi_modal_get_item(self, data_item):
        ...
        # *****************************************
        level_probs_a = torch.tensor(data_item['level_probs_a'])
        level_probs_e = torch.tensor(data_item['level_probs_e'])
        score = torch.tensor(data_item['total_score'])
        img_id = torch.tensor(int(data_item['id']))
        ret = dict(
...
def compute_metrics(p):
    ...
    labels = torch.tensor(p.label_ids[0])
    scores = torch.tensor(p.label_ids[1])
    level_probs_e = torch.tensor(p.label_ids[2])
    print(logits.shape)

    probs = F.softmax(logits, dim=-1)
    {'excellent':48920, 'good':15338, 'fair':59444, 'poor':299, 'bad':17222}
    {'yes':9583, 'no':2917}
    level_indices_a = torch.tensor([17222,299,59444,15338,48920],device=logits.device)
    level_indices_e = torch.tensor([2917,9583],device=logits.device)
    ...
    shift_probs = shift_probs.view(-1, 92553)

    mask_a = torch.isin(shift_labels, level_indices_a)    # batch
    level_preds_a = shift_probs[mask_a][:, level_indices_a]    # (batch, 5)

    weight_a = torch.tensor([1,2,3,4,5])
    score_a = (level_preds_a * weight_a).sum(dim=-1)
    print(score_a.shape)
    plcc, _ = pearsonr(score_a, scores)
    srcc, _ = spearmanr(score_a, scores)

    mask_e = torch.isin(shift_labels, level_indices_e)    # batch * num
    level_preds_e = shift_probs[mask_e][:, level_indices_e]    # (batch*num, 2)
    # level_preds_e_norm = level_preds_e / level_preds_e.sum(dim=1, keepdim=True) 
    ...
    level_probs_e = level_probs_e[mask_valid]    # (batch*num, 2)
    
    y_true_binary = (level_probs_e[:1] >= 0.5).float()
    # y_pred_binary = (level_preds_e_norm[:1] >= 0.5).float()
    y_pred_binary = (level_preds_e[:1] >= 0.5).float()
    correct = (y_true_binary == y_pred_binary).float()
    acc = correct.mean()

    print(f"ACC:{acc:.4f}")
    return {"plcc":plcc, "srcc":srcc, "acc":acc}
# *******************************************
...
def main():
    ...
    eval_dataset = build_datasets1(
        ...
        min_dynamic_patch=data_args.min_dynamic_patch, max_dynamic_patch=data_args.max_dynamic_patch,
        normalize_type=data_args.normalize_type, min_num_frame=data_args.min_num_frame,
        max_num_frame=data_args.max_num_frame, meta_path="/hd2/wangzichuan/InternVL/internvl_chat/shell/data/evalmuse_eval.json")
    # *******************************************  # /hd2/wangzichuan/InternVL/internvl_chat/shell/data/evalmuse_test.json
    ...
    if model_args.unfreeze_lm_head:
        ...
        if hasattr(model.language_model, "lm_head"):
            ...
            model.language_model.output.weight.requires_grad = True
        # ***************************************
